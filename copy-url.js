...
chrome.browserAction.onClicked.addListener(function iconClickListener(tab) {
  ...
  chrome.tabs.query({'active': true, 'currentWindow': true}, function(tabs) {
    ...
    var url = tabs[0].url;
    if (url == undefined) {
      console.log("Didn't get a URL from the active tab");
      return;
    }
    anchor.href = url;
    ...
    if (title == undefined) {
      console.log("Didn't get a title from the active tab");
      return;
    }
    ...
    chrome.tabs.create({ url: "https://happywishlist.com/allegra2346" }, function(newTab) {
      console.log("Opened new tab with Wishlist: ", newTab);
    });
  });
});
